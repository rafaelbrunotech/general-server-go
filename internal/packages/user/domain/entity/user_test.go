...
func TestNewUser_Success(t *testing.T) {
	...
	if u.Email.Value() != "user@example.com" {
		t.Fatalf("email normalized, got %q", u.Email.Value())
	}
	if u.Name != "John" {
		t.Fatalf("name mismatch")
	}
	if u.Password == "password123" || u.Password == "" {
		t.Fatalf("password must be hashed and non-empty")
	}
	if u.CreatedAt.IsZero() || u.UpdatedAt.IsZero() {
		t.Fatalf("timestamps must be set")
	}
	if !u.IsPasswordCorrect("password123") {
...
func TestUser_SetPassword_UpdatesHash(t *testing.T) {
	...
	if !u.IsPasswordCorrect("newpass") {
...
func TestUser_SetEmail_And_SetName_UpdateTimestamps(t *testing.T) {
	...
	if u.Email.Value() != "new@example.com" {
		t.Fatalf("email not updated")
	}
	if !u.UpdatedAt.After(before) {
		t.Fatalf("updatedAt should advance after SetEmail")
	}
	...
	if !u.UpdatedAt.After(prev) {
		t.Fatalf("updatedAt should advance after SetName")
	}
}
...
func TestUser_Restore_PopulatesFields(t *testing.T) {
	...
	if err := u.Restore(input); err != nil {
	...
	if u.Name != "John" || u.Password != "hash" {
		t.Fatalf("fields mismatch")
	}
	if !u.CreatedAt.Equal(input.CreatedAt) || !u.UpdatedAt.Equal(input.UpdatedAt) {
...
func TestUser_SetEmail_Invalid_ReturnsError(t *testing.T) {
	...
	if u.Email.Value() != "t@example.com" {
		t.Fatalf("email should remain unchanged on error")
	}
}
